...
<div id="<?php echo $uid ?>">
	<h3><svg xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;" width="24" height="24" viewBox="0 0 20 20"><rect width="20" height="20" fill="none"/><path fill="currentColor" d="M10.475 2.17a.75.75 0 0 0-.95 0l-2.75 2.25a.75.75 0 0 0 .95 1.16L10 3.72l2.275 1.86a.75.75 0 1 0 .95-1.16zm2.75 13.41l-2.75 2.25a.75.75 0 0 1-.95 0l-2.75-2.25a.75.75 0 0 1 .95-1.16L10 16.28l2.275-1.861a.75.75 0 1 1 .95 1.16M10.75 8.75a.75.75 0 1 1-1.5 0a.75.75 0 0 1 1.5 0M10 12a.75.75 0 1 0 0-1.5a.75.75 0 0 0 0 1.5m6-2.5a1.5 1.5 0 0 1 3 0v1a1.5 1.5 0 0 1-3 0zm1.5-.5a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 1 0v-1a.5.5 0 0 0-.5-.5m-11-1A1.5 1.5 0 0 0 5 9.5v1a1.5 1.5 0 0 0 3 0v-1A1.5 1.5 0 0 0 6.5 8M6 9.5a.5.5 0 0 1 1 0v1a.5.5 0 0 1-1 0zm6 0a1.5 1.5 0 0 1 3 0v1a1.5 1.5 0 0 1-3 0zm1.5-.5a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 1 0v-1a.5.5 0 0 0-.5-.5m-11-1A1.5 1.5 0 0 0 1 9.5v1a1.5 1.5 0 0 0 3 0v-1A1.5 1.5 0 0 0 2.5 8M2 9.5a.5.5 0 0 1 1 0v1a.5.5 0 0 1-1 0z"/></svg> <?php echo $i18n_m('lang_Time_Status'); ?> </h3>

	<div class="time-row">
		<div class="time-label"><?php echo $i18n_m('lang_Local_Time'); ?> (<span id="<?php echo $uid ?>-localTz"></span>)</div>
	...
	<?php if ($serverTimezone): ?>
		<div class="time-row">
			<div class="time-label">
				<?php echo $i18n_m('lang_Server_Time'); ?> (<?php echo htmlspecialchars($serverTimezone); ?>)
			</div>
	...
	<?php else: ?>
		<div class="time-warning">
			⚠ <?php echo $i18n_m('lang_not_set'); ?>.<br>
			<?php echo $i18n_m('lang_Please_define'); ?> <code>gsconfig.php</code>
		</div>
	<?php endif; ?>

	<?php if ($serverISO): ?>
		<div class="time-row diff-row">
			<div class="time-label"><?php echo $i18n_m('lang_Time_Difference'); ?> : </div>
...
function updateAllTimes() {
	...
	if (diffMinutes === 0) {
		text = "<?php echo $i18n_m('lang_Same_timezone'); ?>";
	} else {
		text = hours + "h " + minutes + "m " +
			(diffMinutes > 0 ? "<?php echo $i18n_m('lang_ahead'); ?>" : "<?php echo $i18n_m('lang_behind'); ?>");
	}
